 ...
 * @param options
 */
export const Table = (options?: Partial<{ name: string; timestamps: boolean }>) => (target: Function) => {
...
function PropertyDecorator(options: ColumnDecoratorOptions = {}) {
  return (target: any, propertyName: PropertyType) => {
    ...
    const existingMetadata = Reflect.getMetadata(PROPERTY_ATTRIBUTE_OPTIONS, target?.constructor);
    if (existingMetadata) {
      if (Reflect.has(existingMetadata, propertyName)) {
        ...
        return Reflect.defineMetadata(PROPERTY_ATTRIBUTE_OPTIONS, existingMetadata, target.constructor);
      }
    ...
    Reflect.defineMetadata(PROPERTY_ATTRIBUTE_OPTIONS, newMetadata, target?.constructor);
  };
}
