...
export default class ArraySorter<ItemType> {
  ...
  public sortBy(props: Props = null): ItemType[] {
    ...
    if (props?.hasOwnProperty('desc') && (props as PropObject).desc) {
      ref.sorted = (ref.sorted as Array<string | number | null | undefined | symbol>).reverse();
    }
  ...
  private sortPlainList(data: Array<string | number>) {
    return data.sort(function (next, current) {
      ...
      if ((current as string)?.toLowerCase() > (next as string)?.toLowerCase()) return -1;
      if ((current as string)?.toLowerCase() < (next as string)?.toLowerCase()) return 1;
      return 0;
    });
  }
  ...
  private sortListOfObjects(data: ItemType[], key: string): ItemType[] {
    return data.sort(function (next: { [key: string]: any }, current: { [key: string]: any }) {
      // Sorting by number key
